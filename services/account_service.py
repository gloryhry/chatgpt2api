...
class AccountService:
    ...
    def build_export_items(self, access_tokens: list[str] | None = None) -> list[dict[str, str]]:
        ...
        for account in accounts:
            ...
            access_auth = _nested_dict(access_payload.get("https://api.openai.com/auth"))
            id_auth = _nested_dict(id_payload.get("https://api.openai.com/auth"))
            profile_claim = _nested_dict(access_payload.get("https://api.openai.com/profile"))

            email = (
                _clean_string(account.get("email"))
                or _clean_string(profile_claim.get("email"))
                or _clean_string(id_payload.get("email"))
            )
            account_id = (
                ...
                or _clean_string(account.get("user_id"))
            )
            item = {
                "type": _clean_string(account.get("export_type")) or "codex",
                "email": email,
                "expired": _clean_string(account.get("expired")) or _format_timestamp(access_payload.get("exp")),
                "id_token": id_token,
                "account_id": account_id,
                "access_token": access_token,
                "last_refresh": (
                    _clean_string(account.get("last_refresh"))
                    or _format_timestamp(access_payload.get("iat"))
                    or _format_timestamp(access_payload.get("nbf"))
                ),
                "refresh_token": refresh_token,
            }
